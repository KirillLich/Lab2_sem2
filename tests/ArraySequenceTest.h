...
{
	DynamicArrayTester* tester;
	int NumberOfTests = 3 + 5 + 5;
	...
	void TestGetSubArray_1_5_6_3_9From_1To_3()
	{
		int* array = new int[5];
		array[0] = 1;
		array[1] = 5;
		...
		Sequence<int>* arrayDSub = arrayD.GetSubSequence(1, 3);

		assert(arrayDSub->GetLenght() == 3);
		assert(arrayDSub->Get(0) == arrayD.Get(1));
		assert(arrayDSub->Get(1) == arrayD.Get(2));
		assert(arrayDSub->Get(2) == arrayD.Get(3));
		NumberOfPassedTests++;
	}
	void TestAllDecompositionsArray()
	{
		...
		this->TestGetSubArray_1_5_6_3_9From_1To_3();
	}
...
public:
	...
	{
		int n;
		tester = new DynamicArrayTester(n);
		NumberOfTests += n;
		...
		std::cout << "Array Sequence Tests passed!\n";
